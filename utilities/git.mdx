...
* **Object** := (sha1) -> (headers, body)
  ...
  * The sha1 is always the full content hash (headers + body) and thus is a signature of the object and dependent objects.
...
* **Commit** (Object)
  ...
  * `author` and `committer` headers represent the author and committer of the commit (what a surprise), along with the time at which the event happens. Mostly provided as information, can not and should not be trusted unless you trust the person that provided the information.

* **Tree** (Object)

  A **tree** has no header, only a list of children, one per line. On each line the fields `perms type sha1` are separated by single spaces, then a tab separates the filename (this is exactly what `git cat-file -p <tree>` prints). It represents what a regular filesystem would call a "directory", replacing inodes by hashes so the sub-nodes are identified in a universal namespace instead of a locally determined namespace.
    
  Example:

```
040000 tree 67a00caf3712f4a38c9da6d14a07420530714fb5	doc
...
* Not perfect, but here is an UML-like schema of what is said above
