...
free; add `-d` to keep the focus where it is. `move-window` slides one window to a new index, but
...
message instead of only flagging the window. A related option, `monitor-bell`, flags a window when a
program rings the terminal bell and is `on` by default. Defaults on 3.2a: `monitor-activity off`,
`monitor-silence 0`, `visual-activity off`, `monitor-bell on`.
...
just before, so bouncing to a pane and back costs one keystroke each way. It is the pane-level
...
window name. Splitting, switching windows, or anything that changes the layout unzooms it
automatically.
...
Verified on tmux 3.2a: with two panes side by side, the active pane goes from width 99 to 109 after
`resize-pane -L 10`, and to 80 after `-x 80`; `-y 10` sets it to 10 rows. To rearrange every pane at
once instead of nudging one border, change the layout (see below).
...
drag a border to resize; a right-click on a pane opens a context menu. Verified on tmux 3.2a:
`mouse off` by default, and the copy path is `MouseDrag1Pane` then `MouseDragEnd1Pane`
...
(`%3`) or a `window.pane` form when a target must be unambiguous. Verified on tmux 3.2a: defaults are
`0`/`0`; with the config above a fresh session reports window index `1` and panes `1` and `2`.
...
Qualify the target to stay deterministic: a `window.pane` form (`3.1`), a pane id (`%7`, shown by
`display-panes` and in `#{pane_id}`), or a window name (`build.1`). A pane id is the strongest
choice because it is stable for the life of the pane and is never re-read as a window. Verified on
tmux 3.2a, checking `#{window_index}.#{pane_index}`: with the current window holding panes 0..3,
`-t 3` resolves to `0.3`; with a single pane and a window 7 present, `-t 7` resolves to window 7's
active pane; grow the current window past eight panes and `-t 7` flips back to `0.7`.
