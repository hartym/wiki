...
2x2 grid (99x24 / 100x24 / 99x25 / 100x25 on 200x50). list-keys: prefix Space = next-layout;
prefix M-1..M-5 = select-layout even-horizontal/even-vertical/main-horizontal/main-vertical/tiled. */}

Tip article: [How to reorganize a tmux layout?](/blog/en/tips/2026/07/13/tmux-layouts/)
...
{/* Verification notes (3.2a): list-keys -T prefix shows ':' bound to command-prompt; list-commands
gives command-prompt [-1kiNTW] [-I inputs] [-p prompts] [-t target-client] [template]. */}

Tip article: [How to run a tmux command from inside a session?](/blog/en/tips/2026/07/15/run-tmux-command-interactively/)
...
prints e.g. "resize-pane (resizep) [-DLMRTUZ] [-x width] [-y height] [-t target-pane] [adjustment]". */}

Tip article: [How to discover and search tmux key bindings?](/blog/en/tips/2026/07/16/discover-tmux-key-bindings/)
...
binds, not one-off actions).

Verified on tmux 3.2a.

{/* Verification notes (3.2a): bind r registers as source-file ... \; display-message; list-keys
-T prefix shows the two-command bind. Live test: status-left V1 in the file, edit to V2,
